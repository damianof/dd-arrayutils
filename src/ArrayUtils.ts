...
export const ArrayUtils: IArrayUtilsStatic = class implements IArrayUtils {
  private static getProperty<T, K extends keyof T>(obj: T, key: K) {
    return obj[key]
  }

  public static orderBy<T, K extends keyof T>(values: T[], orderType: K) {
    if ((values || []).length > 1) {
      ;(values || []).sort((a, b) => {
  ...
  public static chunk<T>(values: T[], chunkSize: number): T[][] {
  ...
  public static shuffle<T>(a: T[]): void {
